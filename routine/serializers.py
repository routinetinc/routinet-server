...
class Routine_update(serializers.Serializer):  # /routine/routine(PATCH)
    routine_id    = serializers.IntegerField(max_value=None, min_value=None)
    dow           = serializers.ListField(child=serializers.IntegerField(min_value=0, max_value=6),required = False)
    start_time    = CustomSerializers.TimeStringField(required = False)
    end_time      = CustomSerializers.TimeStringField(required = False)  # 時間未設定タスクを含んだ幅を持たせる
    title         = serializers.CharField(max_length=15, min_length=None, allow_blank=False, trim_whitespace=True, required = False)
    subtitle      = serializers.CharField(max_length=40, min_length=None, allow_blank=True, trim_whitespace=True, required = False)
    icon          = serializers.CharField(max_length=1, min_length=None, allow_blank=True, trim_whitespace=True, required = False)
    is_published  = serializers.BooleanField(required = False)
    is_notified   = serializers.BooleanField(required = False)
    interest_ids  = serializers.ListField(child=serializers.IntegerField(max_value=None, min_value=None), required = False)
