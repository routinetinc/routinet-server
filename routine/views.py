...
class Routine(APIView):
    ...
    def patch(self, request, format=None):
        try:
            datas = get_json(request, serializers.Routine_update)
        except RequestInvalid:
            return make_response(status_code=400)
        try: 
            #データベースからRoutineオブジェクトを取得
            r = Routine.objects.get(id=datas["routine_id"])
...
class Task(APIView):
    ...
    def post(self, request, format=None):
        try:
            ...
            return make_response(status_code=400)
        try: 
            t = Task(routine_id = datas["routine_id"],
                    title         = datas["title"],
                    detail        = datas["detail"],  
                    icon          = datas["icon"],  
    ...
    def patch(self, request, format=None):
        try:
            datas = get_json(request, serializers.Task_update)
        except RequestInvalid:
            return make_response(status_code=400)
        try: 
            t = Task.objects.get(id=datas["task_id"])
